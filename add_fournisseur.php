...
try {
    ...
    if(isset($_POST["submit"])) {
        ...
        if(empty($input_nom)){
           $nom_err = "Veuillez insérer fournisseur";
        ...
        if ($input_selectstatut == "vide"){
            $selectstatut_err = "Veuillez selectionner la statut de votre fournisseur";             
        } else {
...
// DETECTER SI LE FOURNISSEUR A INSERER EXISTE DEJA DANS LE TABLE FOURNISSEUR 
    ...
    if($nombre <> 0 AND (empty($nom_err) && empty($adresse_err) && empty($ville_err) && empty($pays_err) && empty($tel_err) && empty($mail_err) && empty($selectstatut_err) && empty($rcs_err) && empty($stat_err) && empty($nif_err))) {
        ...
        if(empty($nom_err) && empty($adresse_err) && empty($ville_err) && empty($pays_err) && empty($tel_err) && empty($mail_err) && empty($selectstatut_err) && empty($rcs_err) && empty($stat_err) && empty($nif_err) && !isset($_SESSION['fournisseur_existe']) ) {
                ...
                $sql="INSERT INTO fournisseur (ID, NOM, TEL, COURRIEL, ADRESSE, VILLE, PAYS, STATUT, RCS, NIF, STAT, DATE_AJOUT, AJOUTE_PAR)
                ...
                if($ex <> false){
                    ...
                    $_SESSION['alert_title'] = "Création nouveau fournisseur terminée";
                    $_SESSION['alert_text'] = "Votre fournisseur à été ajouté avec succès!";
                    header("location: read_fournisseur.php");
                    exit();
                } else{
                    $_SESSION['alert_icon'] = "error";
                    $_SESSION['alert_title'] = "Création nouveau fournisseur échouée";
        ...
        // SI LE DOUBLON EXISTE MAIS ON INSERE QUAND MEME LE FOURNISSEUR
        elseif (isset($_POST['add_fournisseur'])) {
                ...
                if($ex <> false){
                    ...
                    $_SESSION['alert_title'] = "Création nouveau fournisseur échouée";
                    $_SESSION['alert_text'] = "Un problème est survenu. Veuillez réessayer plus tard ou veuillez vous reconnecter";
                    header("location: add_fournisseur.php");
                    exit();                    
                }
...
<?=template_header('Création de fournisseur')?>
...
<?=template_content('fournisseur')?>
    ...
    <!-- FIN FONCTION -->  
            ...
            <div id="navigation" class="row">
                <h5><a href="index.php"><i class='fas fa-home'></i></a> / Fournisseurs / Création nouveau fournisseur</h5>
                <div class="navigation-retour-active">
                  <a href="read_fournisseur.php"><i class="fas fa-chevron-circle-left"></i></a>
                </div>
            ...
            <div id="cadre-contenu">
                ...
                <div class="container">
                    <form name="myForm" id="form-add" action="add_fournisseur.php" method="post">
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    <input class="form-control" type="text" name="nom" placeholder="FOURNISSEUR *" value="<?php if(isset($_SESSION['nom'])) {echo ucwords($_SESSION['nom']);}?>" oninput="controlInputNOM();" id="nom">
                            ...
                            <?php if( $selectstatut=="professionnel"  ) : ?>
                                <div class="form-group">                                
                                  <select class="custom-select mr-sm-2"  id="SelectStatu" onchange="getSelectValue();" oninput="controlInputSTATUT();" name="selectstatut">
                                    ...
                                    <option value="vide">---CHOISIR LE STATUT DU FOURNISSEUR--- *</option>
               ...
               <!-- TABLEAU FOURNISSEUR --> 
                <table id="table" class="table">
                  ...
                  <form method="post">
                    ...
                    <button type="submit" class="btn btn-danger ml-2 mr-2" name="add_fournisseur">Insérer</button>                    
...
<?php
 ...
 if (!isset($_SESSION['fournisseur_existe'])) {
     // code...
    unset($_SESSION['nom']);
    unset($_SESSION['adresse']);
    unset($_SESSION['ville']);
